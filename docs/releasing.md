...
2. Update only the release metadata deliberately selected for that release:
   ...
   Every release must use the selected exact numeric version string in all active release fields; no shortened changelog exception is permitted.
...
5. Review the `RC_PREPARED` summary and artifact evidence:
   ...
   - exact positive payload and file count derived from `release/payload-manifest.txt`;
