<?php
/**
 * Thumbnail Manager admin interface.
 *
...
/**
 * Render the Thumbnail Manager administration page.
 */
function yotm_manage_thumbnails_page() {
	...
	// Handle the Thumbnail Sizes form save.
	$sizes_saved_notice        = '';
	...
	if ( isset( $_POST['yotm_sizes_save'] ) || isset( $_POST['yotm_save_and_regenerate'] ) ) {
		...
		// Disabled sizes are every registered size not selected as enabled.
	...
	// Get disabled sizes and detect whether the option exists.
	...
	// Sort sizes with the largest width first.
	uasort(
		$sizes,
		function ( $a, $b ) {
			$wa = (int) ( $a['width'] ?? 0 );
	...
	if ( ! $option_exists ) {
		$default_keep = array( 'thumbnail', 'medium', 'large' );
	} else {
